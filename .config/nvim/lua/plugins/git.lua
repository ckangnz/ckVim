...
vim.keymap.set('n', '<leader>2', ':GV --all<CR>', { silent = true, desc = 'Git log (all branches)' })
vim.keymap.set('n', '<leader>@', ':Telescope git_bcommits<CR>', { silent = true, desc = 'Git buffer commits' })
...
vim.keymap.set('n', '<leader>gD', ':Gdiffsplit!<CR>', { silent = true, desc = 'Git diff split (3-way)' })
...
vim.keymap.set('n', '<leader>gp', ':AsyncRun git -c push.default=current push<CR>',
  { silent = true, desc = 'Git push current branch' })
vim.keymap.set('n', '<leader>gP', ':AsyncRun git push -f<CR>',
  { silent = true, desc = 'Git force push' })
vim.keymap.set('n', '<leader>gl', ':AsyncRun git pull<CR>',
  { silent = true, desc = 'Git pull' })

-- Git fetch operations
vim.keymap.set('n', '<leader>gfo', ':AsyncRun git fetch origin<CR>',
  { silent = true, desc = 'Git fetch origin' })
vim.keymap.set('n', '<leader>gfa', ':AsyncRun git fetch --all --prune<CR>',
  { silent = true, desc = 'Git fetch all with prune' })

-- Git status line helper function
local function update_git_status_line()
  if not vim.fn.exists('*FugitiveExtractGitDir') or vim.fn.exists('*FugitiveExtractGitDir') == 0 then
    return ''
  end
